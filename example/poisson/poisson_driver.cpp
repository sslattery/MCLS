...
int main( int argc, char * argv[] )
{
    ...
    int local_num_rows = plist->get<int>("Problem Size");
    ...
    global_columns[0] = global_num_rows-2;
    global_columns[1] = global_num_rows-1;
    values[0] = -0.5;
    values[1] = 1.0;
    A->InsertGlobalValues( global_num_rows-1, 2, 
			   &values[0], &global_columns[0] );
    A->FillComplete();

    // Build the LHS by filling it with the specified fourier mode.
    int k = plist->get<int>("Wave Number");
    Teuchos::RCP<Vector> x = MT::cloneVectorFromMatrixRows( *A );
    double pi = std::atan(1.0)*4.0;
    for ( int i = 0; i < local_num_rows; ++i )
    {
	(*x)[i] = std::sin( i*k*pi / local_num_rows ) + 0.0;
    }

    // Build the RHS. The problem is homogeneous.
    Teuchos::RCP<Vector> b = MT::cloneVectorFromMatrixRows( *A );
    VT::putScalar( *b, 0.0 );
    ...
    // The point-wise error is the 2-norm.
    std::cout << "||e||_2 = " << VT::norm2( *x ) << std::endl;

...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL( AdjointDomain, Typedefs, LO, GO, Scalar )
{
    ...
    typedef MCLS::AdjointTally<Vector> TallyType;
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL( AdjointDomain, NoOverlap, LO, GO, Scalar )
{
    ...
    MCLS::AdjointDomain<VectorType> domain( A, x, plist );
	...
	{
	    HistoryType history( state, x_val );
	    history.live();
    ...
    for ( b_view_iterator = x_view.begin();
	  b_view_iterator != x_view.end();
	  ++b_view_iterator )
    {
